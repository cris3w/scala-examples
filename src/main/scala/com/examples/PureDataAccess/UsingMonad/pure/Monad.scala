...
trait Monad[M[_]] {
  ...
  def cond[X, Y](test: => Boolean,
                 left: => M[X],
                 right: => M[Y]): M[Either[X,Y]] =
    if (test) map(left)(Left(_))
    else map(right)(Right(_))
...
object Monad {
  ...
  object Syntax {
    ...
    def cond[M[_],A,B](test: => Boolean)(left: => M[A], right: => M[B])
                      (implicit M: Monad[M]): M[Either[A, B]] = M.cond(test,left, right)

    implicit class InfixSyntax[M[_], A](ma: M[A])(implicit M: Monad[M]) {

      def flatMap[B](f: A => M[B]): M[B] = M.flatMap(ma)(f)

      def map[B](f: A => B): M[B] = M.flatMap(ma)(s => M.pure(f(s)))
    }
  }
